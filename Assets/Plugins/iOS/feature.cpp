...
fgrpindex_t FeatureSet::add(const string& name, int num0, const string* names0)
{
  ...
  for(int i0 = 0; i0<num0; i0++)
    names.push_back(name + "_" + (names0 == NULL ? Global::intToString(i0) : names0[i0]));
...
fgrpindex_t FeatureSet::add(const string& name, int num0, int num1, const string* names0, const string* names1)
{
  ...
  for(int i0 = 0; i0<num0; i0++)
  {
    ...
    for(int i1 = 0; i1<num1; i1++)
    	names.push_back(name0 + "_" + (names1 == NULL ? Global::intToString(i1) : names1[i1]));
  }
  ...
  for(int i0 = 0; i0<num0; i0++)
  {
    ...
    for(int i1 = 0; i1<num1; i1++)
    {
      ...
      for(int i2 = 0; i2<num2; i2++)
      	names.push_back(name1 + "_" + (names2 == NULL ? Global::intToString(i2) : names2[i2]));
    }
  }
    ...
    for(int i1 = 0; i1<num1; i1++)
    {
      ...
      for(int i2 = 0; i2<num2; i2++)
      {
      	...
      	for(int i3 = 0; i3<num3; i3++)
      		names.push_back(name2 + "_" + (names3 == NULL ? Global::intToString(i3) : names3[i3]));
      }
    }
  }
...
{
	//TODO speed this up dramatically
...
const vector<FeatureSet::PriorMatch>& FeatureSet::getPriorMatches() const
{
	return priorMatches;
}

void FeatureSet::addPriorMatch(PriorMatch match)
{
	priorMatches.push_back(match);
}

void FeatureSet::setPriorIndex(findex_t index)
{
	priorIndex = index;
}

void FeatureSet::addUniformPrior(double wins, double losses)
{
	if(priorIndex < 0) Global::fatalError("priorIndex < 0");

	for(int i = 0; i<numFeatures; i++)
	{
		if(i == priorIndex)
			continue;
		addPriorMatch(PriorMatch(priorIndex,i, losses));
		addPriorMatch(PriorMatch(i,priorIndex, wins));
	}
}
