...
class FeatureSet
{
	...
	STRUCT_NAMED_TRIPLE(findex_t,winner,findex_t,loser,double,weight,PriorMatch);
	...
	int priorIndex;
	vector<PriorMatch> priorMatches;
  ...
  //PRIOR-----------------------------------------------------------
  const vector<PriorMatch>& getPriorMatches() const;

  void addPriorMatch(PriorMatch match);

  void setPriorIndex(findex_t index);

  void addUniformPrior(double wins, double losses);
};
